...
const ExperienceDetail: React.FC<IProps> = ({enabledNext}) => {
    ...
    const handleChange=(event : React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>,index : number, skillIndex?: number)=>{
      ...
      const onSave = async() => {
        setLoading(true);
        const data = {
          experience: experienceList.map(({ company, role, startDate, endDate,responsibilities}) => ({
            company,
            role,
            startDate,
            endDate,
  ...
  return (
    <Box sx={{ padding: '16px' }}>
      {resumeInfo && experienceList.map((entry, index) => (
      ...
      <div className='flex justify-between'>
            ...
            <Button variant="outlined" onClick={AddNewExperience} className="text-primary"> + Add More Experience</Button>
            
            <Button variant="outlined" onClick={RemoveExperience} className="text-primary"> - Remove</Button>
